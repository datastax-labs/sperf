...
def write_config(report, config):
    ...
    append(
        report, "cass drive(s) read ahead", _format_read_ahead(config.get("cass_ra"))
    )
    ...
    append(
        report,
        "disk optimization",
        simple_format(config.get("disk_optimization_strategy")),
    )
    append(
        report,
    ...
    append(
        report,
        "flush writers",
        _format_flush_writers(config.get("memtable_flush_writers")),
    )
    append(
        report,
    ...
    append(
        ...
        simple_format(config.get("concurrent_compactors")),
    )
    memtable_heap_size = config.get(
        "memtable_heap_space_in_mb", config.get("memtable_space_in_mb")
    )
    append(report, "memtable size (heap)", simple_format(memtable_heap_size))
    append(
        report,
        "memtable size (offheap)",
    ...
    append(
        report, "file cache size", simple_format(config.get("file_cache_size_in_mb"))
    )
    append(report, "heap size", simple_format(config.get("heap_size")))
    ...
    append(
        report,
        "busiest table reads",
        format_busiest_table(config.get("busiest_table_reads"), "reads"),
    )
    append(
        report,
        "busiest table writes",
        format_busiest_table(config.get("busiest_table_writes"), "writes"),
    )
    append(
        report, "worst partition in", format_table_loc(config.get("worst_part_size"))
    )
    append(
        report,
        "* max partition size",
