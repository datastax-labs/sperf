...
from pysper.search.queryscore import (
    get_title,
    get_bad_query_summary,
    get_queries_above_threshold,
    QueryParams,
    Parsed,
    generate_report,
)
...
class TestQueryScore(unittest.TestCase):
    """test queryscore module"""

    def test_get_title(self):
        """happy path"""
        self.assertEqual(get_title(True, 5), "top 5 uniquely bad\n------------------\n")

    def test_get_bad_query_summary(self):
        """happy path"""
        bad_queries = [str(x) for x in range(5)]
        self.assertEqual(
            get_bad_query_summary(bad_queries, 100),
            "\nsuspect queries totals: 5/100 - 5.00%\n",
        )

    def test_get_queries_above_threshold(self):
        """happy path"""
        score_threshold = 1
        bad = get_queries_above_threshold(self._add_queries(), score_threshold)
